%% This function will now generate tags for the second Phrase
function TagPhrase2 (object_handle, event) %inputs for this function include all the same variables
%% global variables
global t1
global T1
global t2
...
%Now we will look for the end boundary and end viewing window for the second phrase
...
% Tag 1 loops:    
for i = 1:length(TB_Matrix) 

    if TB_Matrix(i, 1) >= T1; %find the Timing that is closest to T1
...
Maximum = TB_Matrix(tag_bound_1, 2); %The phrase begins when TB is at a maximum. Place the initial maximum position
for i = tag_bound_1 + 1: tag_bound_2
    if Maximum < TB_Matrix(i,2); %if any y position between the two bounds is greater than the initial maximum, then it will be the new maximum
        Maximum = TB_Matrix(i,2);
    end
end

[I, J] = find (TB_Matrix == Maximum); %find the row number for the maximum y value
Tag_1_time = TB_Matrix(I,1); %Note down the time for this maximum y value. This will be the position of the first tag.
...
for newcolumn = 1:18; %newcolumn is the column number from the matrix SensorData; each of these columns has motion data and will be plotted individually vs time
    ...
    accept = uicontrol ('Style', 'pushbutton', 'String', 'Accept Tags', 'units',...
        'normalized', 'Position', [.665 .35 .2 .06], 'callback', {@AcceptPhrase2Tags}); %button that will allow the user to accept the tag positions, save the tag positions and the range of motion
    
   next = uicontrol ('Style', 'pushbutton', 'String', 'Tag Phrase 3', 'units',...
        'normalized', 'Position', [.78 .18 .2 .06], 'callback', {@TagPhrase3}); %button that will allow the user to accept the tag positions, save the tag positions and the range of motion
...
function AcceptPhrase2Tags (object_handle, event)
global Tag_1_time
global Tag_2_time
global Timings
global SensorData
...
fprintf(fileID, '2\t %10.3f\t %10.3f\t %10.3f\t %10.3f\t %10.3f\t \r\n', Tag_1_time, Tag_2_time, RangeX, RangeY, RangeZ);
fclose(fileID);
